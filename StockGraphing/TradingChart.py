...
class TradingChart:
    ...
    def _render_net_worth(self, step_range, times, current_step, net_worths, benchmarks):
        ...
        if benchmarks != None: self._render_benchmarks(step_range, times, benchmarks)
    ...
    def _render_price(self, step_range, times, current_step):
        ...
        #myTimes = [mdates.date2num(pd.to_datetime(d)) for d in times]
        
        #candlesticks = zip(range(current_step),
        ...
        # Plot price using candlestick graph from mpl_finance
        #candlestick(self.price_ax, candlesticks, width=1, colorup="g", colordown="r")
        ...
        color="black",
        fontsize="small")
        self.price_ax.plot(times, self.df['assetClose'].values[step_range], color="black")
    ...
    def render(self, current_step, net_worths, benchmarks, trades, window_size=200):
        ...
        date_col = pd.to_datetime(self.df['Date']).dt.strftime('%d/%m/%Y')
